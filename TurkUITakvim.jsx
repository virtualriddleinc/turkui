    ...
    <div className="relative w-full h-full overflow-hidden select-none group">
      <div
        ...
        style={{
          ...
          background: 'rgba(255, 255, 255, 0.2)', // Biraz daha belirgin seçim alanı
                ...
                <li
                    ...
                    className={`
                        ...
                        ${isActive
                            ...
                            : 'text-white/60 text-[9px] opacity-50 scale-95'}
                    `} // Pasif öğelerin görünürlüğünü white/50'den white/60'a çektim
...
const CalendarContent = ({ initialDate, onDateSelect, onClose }) => {
  ...
  return (
    ...
    >
        {/* AY SEÇİCİ (SOL) - Arka plan koyulaştırıldı, blur artırıldı */}
        <div className={`
            glass-panel-side absolute top-0 right-full mr-2 h-full flex flex-col overflow-hidden z-10
            transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] w-[65px]
            ${isMonthListVisible
                ? 'opacity-100 translate-x-0'
                : 'opacity-0 translate-x-4 pointer-events-none'}
        `}>
          <div className="w-[65px] h-full bg-[#032221]/95 backdrop-blur-3xl rounded-xl border border-white/25 shadow-2xl">
            <WheelPicker
                items={months}
        ...
        {/* TAKVİM (ORTA) - Arka plan opaklığı %95 yapıldı, gölgeler güçlendirildi */}
        <div className="glass-panel-main w-[160px] h-full p-2.5 flex flex-col justify-between z-20 relative bg-[#0f3d3a]/95 rounded-xl border border-white/20 shadow-2xl shadow-black/40 backdrop-blur-2xl">
          ...
          <div className="flex justify-between items-end px-0.5 mb-1 mt-0.5">
            ...
            <div
               ...
               className="group flex items-center gap-1 cursor-pointer select-none"
            >
              <span className="text-[11px] text-white/70 font-medium group-hover:text-white/90 transition-colors">
                {selectedYear}
              </span>
              <div className={`
                    p-0.5 rounded-full border border-white/10 bg-white/5 text-white/50
          ...
          <div className="grid grid-cols-7 mb-1 border-b border-white/10 pb-1">
            {['Pt', 'Sa', 'Ça', 'Pe', 'Cu', 'Ct', 'Pz'].map((day, i) => (
              <div key={day} className={`text-center text-[7px] font-bold uppercase tracking-wide rounded-sm ${i >= 5 ? 'text-white/50' : 'text-white/40'}`}>
                {day}
              </div>
            ))}
          </div>
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${!isSelected && isWeekend ? 'text-white/70' : ''}
                  `}
                >
                  {d.day}
                </button>
        ...
        {/* YIL SEÇİCİ (SAĞ) */}
        <div className={`
            glass-panel-side absolute top-0 left-full ml-2 h-full flex flex-col overflow-hidden z-10
            transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] w-[50px]
            ${isYearListVisible
                ? 'opacity-100 translate-x-0'
                : 'opacity-0 -translate-x-4 pointer-events-none'}
        `}>
          <div className="w-[50px] h-full bg-[#032221]/95 backdrop-blur-3xl rounded-xl border border-white/25 shadow-2xl">
            <WheelPicker
                items={years}
...
// --- ANA BİLEŞEN: TURK UI TAKVİM ---
export default function TurkUITakvim({ value, onChange, placeholder = "Tarih Seçiniz", className = "" }) {
  ...
  return (
    <>
      ...
      <div
        ...
        // Yoğunlaştırılmış Input Arkaplanı: Daha koyu teal ve %95 opaklık.
        className={`
            flex items-center gap-2 w-[200px] p-2 rounded-lg cursor-pointer transition-all duration-300 group
            bg-[#0f4c47]/95 backdrop-blur-xl border border-white/20
            shadow-[0_4px_20px_rgba(0,0,0,0.15)]
            hover:shadow-[0_4px_25px_rgba(45,212,191,0.4)] hover:border-white/40 hover:scale-[1.01]
            ${className}
        `}
      >
          ...
          <span className="text-white/95 text-[13px] font-medium tracking-wide select-none flex-1 truncate drop-shadow-sm">
            {formatDate(internalDate)}
          </span>

          <ChevronRight className="text-white/50 group-hover:text-white/90 group-hover:translate-x-0.5 transition-all" size={14} />
      </div>
